...
setGlobals() {
  ...
  export ORG_DIR="org${USING_ORG}.${MODEL_ID}.com"
  ...
  export PEER0_ORG${USING_ORG}_CA=${ORGANIZATIONS_PATH}/peerOrganizations/${ORG_DIR}/peers/peer0.${ORG_DIR}/tls/ca.crt
...
computeMSPs() {
  ...
  while [ $c -le $ORGS ]
  do
    MSP=Org${c}MSP${MODEL_ID}
    if [ $c -eq 1 ]; then
      MSPs="'${MSP}.member',"
    elif [ $c -eq $ORGS ]; then
      MSPs="$MSPs'${MSP}.member'"
    else 
      MSPs="$MSPs'${MSP}.member',"
    fi
    ((c++))
